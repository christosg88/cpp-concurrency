...
private:
  ...
  private:
    ...
    bucket_iterator find_entry_for(Key const &key) const {
      return std::find_if(_data.begin(), _data.end(), [&](bucket_value const &item) { return item.first == key; });
    }
...
public:
  ...
  explicit threadsafe_lut(unsigned num_buckets = NUM_BUCKETS, Hash const &hasher = Hash())
  ...
  threadsafe_lut &operator=(threadsafe_lut const &other) = delete;
  threadsafe_lut(threadsafe_lut &&other) = delete;
  threadsafe_lut &operator=(threadsafe_lut &&other) = delete;
  ...
  std::map<Key, Value> get_map() const {
    ...
    for (bucket_type const &bucket : _buckets) {
      locks.emplace_back(bucket._mtx);
    }
